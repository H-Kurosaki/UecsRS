...
{
Application->MessageBox("ソケットが他のアプリケーションで占有されているかLANにアクセスできません", "起動エラー", MB_OK);
exit(0);
}
...
recvSpeedHist[count]=recvSpeedSec;
count++;
...
if(startFlag)
{Label8->Caption=IntToStr(recvSpeedMin)+"packet/分";}
else
{Label8->Caption="????packet/分";}



Label5->Caption=IntToStr(recvSpeedSec)+"packet/秒";
recvSpeedSec=0;
...
{
if(RadioButtonSendBcast->Checked)
{WriteDebugLOG("ノードスキャン送信:16529(Broadcast)");UDPSrv02->Broadcast("<?xml version=\"1.0\"?><UECS ver=\"1.00-E10\"><NODESCAN/></UECS>",16529);}
else
{WriteDebugLOG("ノードスキャン送信:16529");UDPSrv02->Send(EditSendIP->Text,16529,"<?xml version=\"1.0\"?><UECS ver=\"1.00-E10\"><NODESCAN/></UECS>");}
...
{
if(RadioButtonSendBcast->Checked)
{WriteDebugLOG("CCMスキャン送信:16529(Broadcast)");UDPSrv02->Broadcast("<?xml version=\"1.0\"?><UECS ver=\"1.00-E10\"><CCMSCAN/></UECS>",16529);}
else
{WriteDebugLOG("CCMスキャン送信:16529");UDPSrv02->Send(EditSendIP->Text,16529,"<?xml version=\"1.0\"?><UECS ver=\"1.00-E10\"><CCMSCAN/></UECS>");}
}
else
{
if(RadioButtonSendBcast->Checked)
{WriteDebugLOG("CCMスキャン送信:16529(Broadcast)");UDPSrv02->Broadcast("<?xml version=\"1.0\"?><UECS ver=\"1.00-E10\"><CCMSCAN page=\""+IntToStr(CSpinScanPage->Value)+"\"/></UECS>",16529);}
else
{WriteDebugLOG("CCMスキャン送信:16529");UDPSrv02->Send(EditSendIP->Text,16529,"<?xml version=\"1.0\"?><UECS ver=\"1.00-E10\"><CCMSCAN page=\""+IntToStr(CSpinScanPage->Value)+"\"/></UECS>");}
}
}
//---------------------------------------------------------------------------
...
{
if(RadioButtonSendBcast->Checked)
{WriteDebugLOG("CCM提供者サーチ送信:16521(Broadcast)");UDPSrv02->Broadcast("<?xml version=\"1.0\"?><UECS ver=\"1.00-E10\"><SEARCH type=\""+EditSrchCCM->Text+"\" room=\"0\" region=\"0\" order=\"0\"/></UECS>",16521);}
else
{WriteDebugLOG("CCM提供者サーチ送信:16521");UDPSrv02->Send(EditSendIP->Text,16521,"<?xml version=\"1.0\"?><UECS ver=\"1.00-E10\"><SEARCH type=\""+EditSrchCCM->Text+"\" room=\"0\" region=\"0\" order=\"0\"/></UECS>");}
...
try{
  ...
  if(myNICCount!=niccount){
                        for(int i=0;i<niccount;i++)
                                {MemoLog->Lines->Add("自分のIPアドレス:"+myIpAddrList[i]);}
                        }
  myNICCount=niccount;
